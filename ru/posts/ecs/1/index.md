...
Как можно заметить данные довольно разреженные и хранить их в двухмерном массиве будет не очень эффективно, особенно если учесть что это, скорее всего будут структуры, а не классы (классы могут иметь разный размер, но на это плевать - т.к. храниться они будут где-то рандомно в куче), да еще и разного размера, но об этом позже - сейчас разбираемся с концепциями.
...
На практике - обычно система представляет из себя интсанс класса реализующего интерфейс ISystem с методом Update, который пробрасывается в мир и хранится в нём. Это решает ряд таких проблем как:
