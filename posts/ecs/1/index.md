...
layout: doc
lang: en-US
title: '1.1 Theory Once (or ECS for Dummies)'
...
Components are the building blocks from which an entity is constructed. It is the set of components that defines what an entity is. At the same time, these building blocks contain only data describing the state of the entity, but do not contain logic.
...
In the simplest version, the data can be represented as a two-dimensional array or table, where each column is a component type, and each row is the index of our entity, something like this:
...
... the scale is reduced, but in a medium-complex project, it's a hundred or two types of components and several thousand entities.

> Very similar to a table in a relational DB with the only difference that each cell is a separate data structure.
...
2. To process the component's data, it is necessary to retrieve it, which with a large number of entities can significantly impact performance. Moreover, based on point 1 - information about the presence of a component can be stored separately from the data and in a different form, for example, in a bitmask.
...
> In some frameworks, this is the only available mechanism to do something with entities en masse, except for direct references to the entities themselves.

## System

A system - from canonical ECS, is the logic of data manipulation described (ideally) as a static function taking references to specific components as arguments and returning no values.
...
4. The ability to "disable" systems - sometimes it can be useful.
...
Hence the approach of "putting ducks with ducks, and apples with apples" was born.

This approach allows processing both ducks and apples separately, as well as together if they are both edible.

From a technical point of view, an archetype is a subset of components based on composition. The illustration below clearly shows what a world structure using archetypes looks like:

![3](3.png)

More complex, scarier, but it allows iterating only through entities with the required set of components, ignoring others.

But there is also a price, direct access through the entity is burdened with extra references to various caches, mappings, sparse sets, etc., which negatively affects the performance of atomic operations.

> For example, if our system needs all entities with components 1 and 3, the iteration will occur only through archetype 4.

> If by components 3 and 6 - through archetypes 1 and 4 respectively.
...
From the API point of view, it behaves like a component, you can also make selections, spin in systems, filter.
